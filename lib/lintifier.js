...
function lint(err, pathname) {
    ...
    // encoding as a string param for earlier versions of node
    fs.readFile(pathname, 'utf8', onread);
}

function isLintable(err, pathname, stat) {
    if (stat.isFile() && pathname.match(/\.(js|json)$/)) {
        return 'lintable';
    }
}

function main(sources, exclude, cb) {
    var scan = new Scan(exclude, isLintable);
